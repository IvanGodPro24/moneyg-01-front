...
import { UserModalProps, UserModalValues } from "./UserModal.types";

const validationSchema: Yup.ObjectSchema<UserModalValues> = Yup.object({
  name: Yup.string()
...
const UserModal = ({
  ...
  const handleSubmit = (values: UserModalValues) => {
    const isNameUnchanged = name && values.name.trim() === name.trim();
          ...
          <Form className={css.form}>
            <label className="relative" htmlFor={photoId}>
              ...
              {avatar ? (
                ...
                <span className={clsx(css.avatar, "relative")}>
                  {name && name[0]}
                </span>
              )}
